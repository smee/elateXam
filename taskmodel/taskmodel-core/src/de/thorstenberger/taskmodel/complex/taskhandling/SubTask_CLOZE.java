 ...
 */
public class SubTask_CLOZE extends SubTask {

	private ComplexTaskDefType.CategoryType.ClozeTaskBlock block;
	private ClozeSubTaskDefType clozeSubTaskDef;
	private ComplexTaskHandlingType.TryType.PageType.ClozeSubTaskType clozeSubTask;
	
	/**
	 * @param id
	 */
	public SubTask_CLOZE( ComplexTaskDefType.CategoryType.ClozeTaskBlock block,
			ClozeSubTaskDefType clozeSubTaskDef,
			ComplexTaskHandlingType.TryType.PageType.ClozeSubTaskType clozeSubTask ) {
		
		super( clozeSubTask.getRefId() );
		this.clozeSubTaskDef = clozeSubTaskDef;
	...
	public void addToPage(PageType page) {
		page.getMcSubTaskOrClozeSubTaskOrTextSubTask().add( clozeSubTask );
	...
	public void setVirtualNum( String virtualNum ){
		clozeSubTask.setVirtualNum( virtualNum );
	}
	...
	public void doAutoCorrection(){
		...
		float negativePoints = block.getClozeConfig().getNegativePoints();
	...
	public void doManualCorrection( CorrectionSubmitData csd ){
	    ...
	    CorrectionSubmitData_CLOZE csdz = (CorrectionSubmitData_CLOZE) csd;
	    
	    Gap[] gaps = getGaps();
	    for( int i=0; i<gaps.length; i++ ){
	        if( !gaps[i].isAutoCorrected() )
	            gaps[i].setManualCorrection( csdz.isCorrect( i ) );
	    }
	...
	public class Gap{
		...
		public Gap( int index, ComplexTaskHandlingType.TryType.PageType.ClozeSubTaskType.GapType gap,
		...
		private boolean isIgnoreCase(){
			...
			else if( block.getClozeConfig().isSetIgnoreCase() )
				return block.getClozeConfig().isIgnoreCase();
			else
